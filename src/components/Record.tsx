...
import {
  ...
  IconButton, Divider, List, ListItemButton, ListItemText, ListItemIcon
} from '@mui/material';
...
}: { material: Material; isSelected: boolean; onSelect: (m: Material) => void }) {
  return (
    <Card
    ...
    >
      ...
      <DialogContent>
        <TextField
          ...
          sx={{ mt: 1, '& .MuiOutlinedInput-root': { borderRadius: '12px', backgroundColor: '#f9f9f9' } }}
        />
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 2 }}>
        <Button onClick={onClose} sx={{ color: '#666', fontWeight: 'bold' }}>キャンセル</Button>
        <Button onClick={() => { onChange(local); onClose(); }} variant="contained" disableElevation
...
function DurationDialog({
  ...
  useEffect(() => { if (open) { setLocalH(hours); setLocalM(minutes); } }, [open]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth
      PaperProps={{ sx: { borderRadius: '20px', p: 1 } }}
    >
      <DialogTitle sx={{ fontWeight: 'bold', pb: 1 }}>勉強時間を入力</DialogTitle>
      <DialogContent>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1 }}>
          <TextField
            ...
            size="small" label="時間"
            inputProps={{ min: 0, max: 24, style: { textAlign: 'center', fontSize: '22px', fontWeight: 'bold' } }}
          ...
          <TextField
            ...
            size="small" label="分"
            inputProps={{ min: 0, max: 59, style: { textAlign: 'center', fontSize: '22px', fontWeight: 'bold' } }}
            sx={{ flex: 1, '& .MuiOutlinedInput-root': { borderRadius: '12px', backgroundColor: '#f9f9f9' } }}
          />
        </Box>
        {total > 0 && (
...
function PagesDialog({
  ...
  open: boolean; onClose: () => void; value: string; onConfirm: (v: string) => void;
}) {
  const [local, setLocal] = useState(value);
  useEffect(() => { if (open) setLocal(value); }, [open]);
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1 }}>
          <TextField
            ...
            inputProps={{ min: 0, style: { textAlign: 'center', fontSize: '22px', fontWeight: 'bold' } }}
            sx={{ '& .MuiOutlinedInput-root': { borderRadius: '12px', backgroundColor: '#f9f9f9' } }}
          />
          <Typography sx={{ fontWeight: 'bold', color: '#666', flexShrink: 0 }}>ページ</Typography>
        </Box>
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 2 }}>
        <Button onClick={onClose} sx={{ color: '#666', fontWeight: 'bold' }}>キャンセル</Button>
        <Button onClick={() => { onConfirm(local); onClose(); }} variant="contained" disableElevation
          sx={{ borderRadius: '8px', fontWeight: 'bold', px: 3 }}>
          決定
        </Button>
      </DialogActions>
...
function ManualInputTab({
  ...
  // 最新の save 関数を ref に登録（レンダーごとに更新）
  useEffect(() => {
    saveFnRef.current = () => onSave(recordDatetime, hours, minutes, pages, memo, image);
  });

  // totalMinutes が変わったら親に通知
  useEffect(() => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%', pt: 2 }}>
        ...
        rightSlot={
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }} onClick={(e) => e.stopPropagation()}>
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); setRecordDatetime(nowDatetimeLocal()); }}
              sx={{ borderRadius: '8px', fontWeight: 'bold', fontSize: '11px', py: 0.4, px: 1.2, minWidth: 0, whiteSpace: 'nowrap' }}
            >
              現在時刻
            </Button>
            <KeyboardArrowRightIcon sx={{ color: '#bbb', fontSize: '20px' }}
              onClick={() => setIsDateDialogOpen(true)} />
          </Box>
        }
      />
      ...
      <CompactCell
        ...
        value={pages ? `${pages} ページ` : undefined}
        placeholder="学習量を入力（ページ数）"
        onClick={() => setIsPagesDialogOpen(true)}
        highlight={!!pages && parseInt(pages) > 0}
      />
      ...
      {image && previewUrl ? (
        ...
        >
          ...
          <Typography sx={{ fontWeight: 'bold', fontSize: '13px', color: '#1A73E8', flexGrow: 1,
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 1, px: 0.5 }}>
          <NoteAltOutlinedIcon sx={{ fontSize: '13px', color: '#888' }} />
          <Typography variant="caption" sx={{ fontWeight: 'bold', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', fontSize: '10px' }}>
            メモ（任意）
          </Typography>
        </Box>
        <TextField
          placeholder="今日の勉強メモ..." value={memo}
          onChange={(e) => setMemo(e.target.value)}
          fullWidth multiline rows={4} disabled={isSaving}
          sx={{ '& .MuiOutlinedInput-root': { borderRadius: '12px', backgroundColor: '#f9f9f9' } }}
        />
      </Box>
      ...
      <PagesDialog
        ...
        value={pages} onConfirm={setPages}
      />
    </Box>
  );
}
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, width: '100%', pt: 2 }}>
      ...
      <ListItemButton onClick={onOpenMaterialDialog} sx={{
        ...
        border: selectedMaterial ? '2px solid #1A73E8' : '1px solid #e8e8e8',
        backgroundColor: selectedMaterial ? '#f0f4fd' : '#fafafa',
        px: 2, py: 2, minHeight: '72px', transition: '0.15s',
        '&:hover': { backgroundColor: selectedMaterial ? '#e8f0fe' : '#f0f0f0', borderColor: '#1A73E8' },
      }}>
        <ListItemIcon sx={{ minWidth: 40, color: selectedMaterial ? '#1A73E8' : '#aaa' }}>
          {selectedMaterial && selectedMaterial !== 'none' ? (
            <img src={(selectedMaterial as Material).image} alt=""
              style={{ height: '36px', width: 'auto', objectFit: 'contain' }} />
          ) : (
            <MenuBookOutlinedIcon sx={{ fontSize: '28px' }} />
          )}
        </ListItemIcon>
        <ListItemText
          primary={
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
              {selectedMaterial && selectedMaterial !== 'none' && (
                <Chip label={(selectedMaterial as Material).categoryName} size="small" sx={{
                  backgroundColor: (selectedMaterial as Material).colorCode, color: '#fff',
                  fontSize: '9px', height: '16px',
                  '& .MuiChip-label': { px: 0.8 }
                }} />
              )}
              <Typography sx={{ fontWeight: 'bold', fontSize: '15px', color: selectedMaterial ? '#1A73E8' : '#bbb' }}>
                {materialLabel() ?? '教材を選択（任意）'}
              </Typography>
            </Box>
          }
        />
        <KeyboardArrowRightIcon sx={{ color: selectedMaterial ? '#1A73E8' : '#bbb', fontSize: '20px' }} />
      </ListItemButton> */}
...
export default function Record() {
  ...
  // useCallback で安定した参照を作成（子の useEffect 依存配列に入れるため）
  ...
  const fetchMaterials = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('materials')
        .select(`id, title, image_url, categories ( name, color_code, sort_order )`)
        .eq('status', 'active')
  ...
  const handleSave = async (
    ...
    try {
      ...
      const materialId = selectedMaterial && selectedMaterial !== 'none'
      ...
      const label = selectedMaterial === 'none' ? '教材なし'
        : selectedMaterial ? `「${(selectedMaterial as Material).name}」` : '教材なし';
      alert(`${label}の記録を保存しました！`);
