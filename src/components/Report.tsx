...
function TrendDetailsView({
  onBack, allLogs, initialViewMode,
}: {
  onBack: () => void;
  allLogs: LogEntry[];
  initialViewMode: TrendViewMode;
}) {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
  const chartColors = theme.palette.chart;

  const [viewMode, setViewMode] = useState<TrendViewMode>(initialViewMode);
  ...
  return (
    <Box sx={{
      ...
      pb: isMobile ? 'calc(56px + env(safe-area-inset-bottom) + 24px)' : 2 // 🌟 ここを追加
    }}>
                          ...
                          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                            ...
                            {mat.image ? (
                              <Box component="img" src={mat.image} sx={{
                                width: isMobile ? 24 : 30, height: isMobile ? 34 : 42, objectFit: 'cover', // 🌟 スマホ時は画像も小さく
                                borderRadius: '4px',
                        ...
                        {barData.map(d => {
                          ...
                          return (
                            <TableCell key={d.date} align="center" sx={{
                              ...
                              {/* 🌟 0分のときは「0分」、スマホなら「1h30m」のようにスリムに表示 */}
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: 0, maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
      {isLoading ? (
        ...
        <TrendDetailsView
          ...
          allLogs={allLogs}
          initialViewMode={period === '1y' ? 'month' : period === '30d' ? 'week' : 'day'}
        />
      ) : (
        /* 通常のレポート画面 */
        <>
          ...
          <Box sx={{
              ...
              {statItems.map((item, i) => (
                ...
                >
                  {/* タイトルを上に配置 */}
                  <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 'bold', fontSize: isMobile ? '10px' : '12px', textAlign: 'center', lineHeight: 1.2 }}>
                    {item.label}
                  </Typography>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    <Box sx={{ position: 'relative', width: '100%' }}>
                      <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', pointerEvents: 'none', zIndex: 0 }}>
                        ...
                        <Typography sx={{ fontWeight: 'bold', fontSize: '16px', color: 'text.primary', lineHeight: 1.3 }}>
